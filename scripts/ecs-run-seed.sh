#!/usr/bin/env bash
# Seed demo workspace + credits as a one-off ECS Fargate task (same image/network as API).
# Usage: ./scripts/ecs-run-seed.sh SkoutDev
...
TASK_ARN="$(aws ecs run-task \
  ...
  --overrides "{\"containerOverrides\":[{\"name\":\"${CONTAINER_NAME}\",\"command\":[\"node\",\"/app/node_modules/@skout/db/dist/seed.js\"]}]}" \
...
if [ "$EXIT_CODE" != "0" ]; then
  echo "Seed failed (exit ${EXIT_CODE}): ${STOP_REASON}"
