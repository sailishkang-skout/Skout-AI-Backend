...
| 3 | Fail-closed RBAC (SkoutDev) | Permissions enforced for real users, not only local | Approve yes/no; run backfill; set `RBAC_ENFORCEMENT_ENABLED=true` | Eng lead + Ops | **Complete 2026-08-25** — Yes. Backfill ran (14 tenants / 14 member grants). `RBAC_ENFORCEMENT_ENABLED=true` live on SkoutDev `api` + `crm` (TD :169 / :83). API health 200. | 2026-08-25 |
...
| 7 | Live SLO dashboards + paging | Targets exist in code; no live burn alerts without Datadog | Datadog access or `DD_API_KEY` + import dashboard; name on-call owner | Ops / SRE | **SkoutDev:** secret + APM path done. **On-call owner: Neeraj.** Dashboard JSON ready (`docs/ops/datadog-slo-dashboard.json`); API import needs `DD_APP_KEY` (not in secret — 401 with API key only). **On-call imports via Datadog UI** (Dashboards → New → Import JSON) or add `DD_APP_KEY` to `SkoutDev/datadog` then re-run import. | 2026-08-25 (on-call named; UI import by owner) |
...
- [#7] ~~On-call named (Neeraj)~~; still UI-import SLO dashboard (or add `DD_APP_KEY`)  
