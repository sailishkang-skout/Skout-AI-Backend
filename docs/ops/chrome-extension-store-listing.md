...
| Packaged zip | `apps/chrome-extension/skout-extension-v0.8.2.zip` |
...
| Version | 0.8.2 (Manifest V3) |
| Short description | Capture LinkedIn profiles into Skout lists, enrich contacts, and support sequence outreach. |
| Permissions justification | `storage` settings; `activeTab`/`scripting` LinkedIn DOM capture; `sidePanel` UI; `tabs` profile/tab detection (`tabs.onUpdated` — not `webNavigation`); `alarms` session refresh; host perms for LinkedIn + Skout + API Gateway |
